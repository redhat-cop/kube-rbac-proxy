...
## also create latest if desired
TAG_LATEST="${TAG_LATEST:-false}"

if [ "$TAG_LATEST" = true ] ; then
  # Create manifest to join all images under one virtual tag
  MANIFEST="docker manifest create -a ${QUAY_PATH}:latest"
  for arch in ${CPU_ARCHS}; do
    MANIFEST="${MANIFEST} ${QUAY_PATH}:${VERSION}-${arch}"
  done
  eval "${MANIFEST}"
  ...
  # Annotate to set which image is build for which CPU architecture
  for arch in ${CPU_ARCHS}; do
    docker manifest annotate --arch "${arch}" "${QUAY_PATH}:latest" "${QUAY_PATH}:${VERSION}-${arch}"
  done
  docker manifest push "${QUAY_PATH}:latest"
fi
